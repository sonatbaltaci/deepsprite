...
N_TRANSFORMATION_PREDICTIONS = 8
N_CLUSTER_SAMPLES = 5
MAX_GIF_SIZE = 64
VIZ_HEIGHT = 300
VIZ_WIDTH = 500
...
class Trainer:
    ...
    def __init__(self, cfg, run_dir, save):
        ...
        self.print_and_log_info(
            "Found {} classes, {} train samples, {} val samples".format(
                self.n_classes, len(train_dataset), len(val_dataset), len(test_dataset)
            )
        )
        ...
        self.test_loader = DataLoader(
            test_dataset, batch_size=self.batch_size, num_workers=self.n_workers
        )
        self.print_and_log_info(
        ...
        # Train metrics
        metric_names = ["time/img", "loss_rec", "loss_em", "loss_bin", "loss_freq", "tau"]
        metric_names += [f"prop_clus{i}" for i in range(self.n_clusters)]
        metric_names += [f"proba_clus{i}" for i in range(self.n_clusters)]
    ...
    def single_train_batch_run(self, images, masks):
        ...
        with torch.no_grad():
            ...
            if hasattr(self.model, "proba"):
                winners = class_prob * class_oh # proba of assigned clusters (L, K, B)
                probabilities = (
                    winners.sum(-1).cpu().numpy() / class_oh.sum(-1).cpu().numpy()
                ).squeeze()
    ...
    def save_metric_plots(self):
        ...
        # Losses
        losses = list(filter(lambda s: s.startswith("loss_rec"), self.train_metrics.names))
        df = df_train.join(df_val[["loss_val"]], how="outer")
        fig = plot_lines(df, losses + ["loss_val"], title="Loss")
        fig.savefig(self.run_dir / "loss.pdf")
        reg_losses = ["loss_em", "loss_bin", "loss_freq"]
        if any([l in self.train_metrics.names for l in reg_losses]):
            reg_losses = list(filter(lambda s: s in self.train_metrics.names, reg_losses))
            fig = plot_lines(df, reg_losses, title="Regularization losses")
            fig.savefig(self.run_dir / "regularization_losses.pdf")
    ...
    def segmentation_qualitative_eval(self):
        ...
        for j in range(N):
            ...
            if self.pred_class:
                ...
                # NOTE: Following code segment is for visualization of weighted compositions per layer for each class.
                # Commented for computational simplicity, comment out to utilize.
                """
