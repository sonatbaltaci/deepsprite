...
class _AbstractMultiObjectDataset(TorchDataset):
    ...
    def __init__(self, split, **kwargs):
        ...
        #if self.eval_mode:
        #    self.indices = indices[int(self.N*0.85):]
        #elif split == "val":
        #    self.indices = indices[int(self.N*0.7):int(self.N*0.85)]
        #else:
        #    self.indices = indices[:int(self.N*0.7)]
        if split == 'val':
            with use_seed(42):
                self.indices = np.random.choice(range(self.N), 100, replace=False)
        else:
